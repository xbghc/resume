...
const projectList = computed({
  get: () => store.data.projects ?? [],
  set: (val) => { store.data.projects = val }
})
        ...
        >
          <div class="flex justify-between items-center">
            <div class="flex items-center gap-2">
              <span class="drag-handle cursor-move text-gray-400 hover:text-gray-600 print:hidden">⋮⋮</span>
            ...
            <div class="flex items-center gap-2">
              <button
                ...
                @click="collapsedItems[project._uid] = !collapsedItems[project._uid]"
                :title="collapsedItems[project._uid] ? t('action.expand') : t('action.collapse')"
              >
                {{ collapsedItems[project._uid] ? '+' : '−' }}
              </button>
              <button
                class="text-sm px-2"
          ...
          <template v-if="!collapsedItems[project._uid]">
            <input
